...
The project implements four classic divide-and-conquer algorithms:
...
Closest Pair also grows as Θ(n log n) but with higher constants.

Recursion depth is consistent with theoretical bounds: MergeSort logarithmic, QuickSort O(log n) on average, Deterministic Select shallow due to smaller-side recursion.

Discussion

Constant factors vary due to caching, JVM optimizations, and garbage collection.

QuickSort can outperform MergeSort in practice despite both being Θ(n log n).

Deterministic Select is slower than randomized selection in practice but guarantees linear time.

Closest Pair is more costly in practice because of sorting and strip management, although still Θ(n log n).
...
Performance differences are primarily due to constant factors and runtime environment behavior.
